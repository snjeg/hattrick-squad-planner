  ...
  return (
    <section className="contribution-card roster-scenarios" aria-labelledby="roster-scenarios-heading">
      ...
      {evaluated && <div className="roster-scenario-results">
        ...
        <div className="table-scroll"><table><thead><tr><th>Checkpoint</th><th>Squad score delta</th><th>Peak delta</th><th>Depth delta</th><th>Wages delta</th><th>Cash delta base</th><th>Net transfers</th><th>Training capacity</th><th>Roster</th></tr></thead><tbody>{evaluated.checkpoints.map((item) => <tr key={item.checkpoint_id}><th><span className={item.checkpoint_id === 'current' ? 'state-badge badge-current' : 'state-badge badge-projected'}>{item.checkpoint_id === 'current' ? 'Current' : 'Projected'}</span>{item.label}</th><td>{signed(item.delta_vs_baseline?.composite_score ?? null)}</td><td>{signed(item.delta_vs_baseline?.peak_strength ?? null)}</td><td>{signed(item.delta_vs_baseline?.depth ?? null)}</td><td>{item.delta_vs_baseline ? money(item.delta_vs_baseline.weekly_wages) : '-'}</td><td>{item.delta_vs_baseline ? money(item.delta_vs_baseline.cash.base) : '-'}</td><td>{money(item.finance.cumulative_transfer_balance.base)}</td><td>{item.training.consumed_capacity.toFixed(1)}/{item.training.meaningful_capacity}<small>{item.training.unused_capacity.toFixed(1)} unused · {item.training.beneficiaries} beneficiaries</small></td><td>{item.metrics.roster_size}</td></tr>)}</tbody></table></div>
